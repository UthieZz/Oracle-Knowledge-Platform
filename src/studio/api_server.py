...
def upload_file():
    ...
    payload = {
        ...
        "job_id": saved[0]["filename"],
        "tenant_id": request.form.get("tenant_id", "default"),
        "silo_id": request.form.get("silo_id", "default"),
        "files": saved,
        "errors": errors,
    }
...
def run_compile():
    ...
    if package:
        ...
        return jsonify({"status": "Success", "message": "Compilation complete.", "job_id": "compile"})
    else:
        pipeline_service.status = "Idle"
...
def get_pipeline_status():
    ...
    if isinstance(status, dict):
        status.setdefault("job_id", request.args.get("job_id", ""))
        status.setdefault("progress", 100 if status.get("status") == "Idle" else 50)
        status.setdefault("message", status.get("status", ""))
        # Map Idle -> completed for Studio progress UI when idle after compile
